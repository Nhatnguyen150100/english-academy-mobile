...
import { IChapter } from "@src/types/chapter.types";
import { chapterService } from "@src/services";

function ChapterDetail() {
  const navigation = useNavigation<StackNavigationProp<CourseStackParams>>();
  const route = useRoute<RouteProp<CourseStackParams, Routes.ChapterDetail>>();
  const chapterId = route?.params.chapterId;
  const [chapterDetail, setChapterDetail] = useState<IChapter>();
  const [loading, setLoading] = useState<boolean>(false);

  const handleChapterDetail = async () => {
    if (!chapterId) {
      Toast.show({
        text1: "Chapter ID is required",
        type: "error",
      });
      return;
    }
    try {
      setLoading(true);
      const rs = await chapterService.getChapterDetail(chapterId);
      ...
      <View style={styles.container}>
        <View style={styles.courseHeader}>
          <MaterialCommunityIcons
            ...
            color={colors.primary}
          />
          <Text style={styles.courseTitle}>{chapterDetail?.title}</Text>
          ...
          <View style={styles.courseMeta}>
            <View style={styles.metaItem}>
              ...
              <Text style={styles.metaText}>
                Created {formatDate(chapterDetail?.createdAt ?? "")}
              </Text>
            </View>
            ...
            <View style={styles.metaItem}>
              ...
              />
              <Text style={styles.metaText}>
                {chapterDetail?.exams.length} exams
              </Text>
            </View>
          </View>
        </View>
        ...
        <Visibility
          visibility={chapterDetail}
          suspenseComponent={loading ? <LoadingScreen /> : null}
        >
          <FlatList
            data={chapterDetail?.exams}
